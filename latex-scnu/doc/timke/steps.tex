...
这一阶段类似TLS协议中的证书预分发，确保客户端能够获取并验证服务器的公钥。预共享阶段的安全性依赖于公钥分发机制的安全性，如证书机构的可信度。实现支持多种公钥分发方式，包括配置文件存储、命令行参数传递和系统证书库集成等。


\subsubsection{第一阶段：临时密钥协商与 0-RTT 传输}

第一阶段的主要目标是快速建立初始会话密钥，支持0-RTT数据传输。这一阶段直接影响用户感知的连接建立延迟，因此需要高效实现。具体流程如下：
...
第一阶段结束后，双方已经建立了临时会话密钥$K_{tmp}$，客户端能够在第一个往返时间内发送加密的应用数据，实现0-RTT特性。对需要立即发送数据的场景（如Web页面加载、API调用）尤为重要，能显著减少用户感知的延迟。
...
第二阶段结束后，双方建立了具有更强安全性的主会话密钥$K_{main}$，该密钥结合了双方的共享密钥信息，具有Stage-2弱前向安全性。与第一阶段相比，即使服务器的长期密钥泄露，只要攻击者不能主动干预通信过程，主会话密钥仍然安全，为长期会话提供了额外的安全保障。

\subsubsection{密钥派生机制}

TIMKE协议的核心是两个密钥派生的步骤，分别用于生成临时会话密钥和主会话密钥，不仅确保了密钥的唯一性和随机性，还进行了会话的上下文绑定，防止中间人攻击和会话混淆：
...
这一派生过程结合了服务器公钥$pk_S$、封装密文$C_1$和共享密钥$K_1$三个关键元素，通过密码学哈希函数$H_1$导出临时会话密钥。确保只有持有正确服务器私钥$sk_S$的一方才能计算出相同的$K_{tmp}$，实现了隐式身份认证。将服务器公钥包含在派生过程中可以防止中间人攻击，而封装密文的包含则确保了密钥的唯一性。
...
主会话密钥的派生结合了更全面的上下文信息，包括服务器公钥$pk_S$、客户端临时公钥$epk_C$、两个封装密文$C_1$和$C_2$，以及两个共享密钥$K_1$和$K_2$。确保主会话密钥的安全性不仅依赖于服务器的长期密钥，还依赖于临时生成的密钥信息，从而提供弱前向安全性。即使服务器长期密钥泄露，攻击者仍需要获取临时密钥才能恢复主会话密钥。

在实现中，两个哈希函数均使用SHA3-512，但通过不同的域分隔符确保它们的输出相互独立，符合现代密码协议的最佳实践，以防止不同上下文中使用相同哈希函数导致安全问题。