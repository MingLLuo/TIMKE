...
基于前述架构设计，本节详细介绍OW-ChCCA-KEM的核心数据结构和算法。我们严格遵循理论方案设计，同时考虑了计算效率和内存使用优化。

\subsubsection{数据结构设计}

核心数据结构包括公钥、私钥、密文以及支持矩阵运算的辅助结构：
...
数据结构通过精心设计的序列化与反序列化机制支持二进制编码，以便网络传输和持久化存储。序列化格式采用紧凑表示，以最小化传输开销。

需要指出，共享参数在许多 KEM 的实现中都是预先制定好的，不同安全级别下有不同的设置，指定一个理论方案的现实参数需要严谨且持久的分析测试，在尚未确定具体参数前，我们在实现中将共享参数作为公钥的一部分，所占空间与其他元素相比可忽略不计。
...
密钥封装算法（Algorithm \ref{alg:encap}）由发送方执行，将随机生成的密钥材料封装为密文。首先生成随机种子$r$，然后通过扩展函数$G$派生出向量$s$、扰动采样参数$\rho$和二元向量$h_0$、$h_1$。利用LWE问题的特性，算法构造样本$\mathbf{x}$和辅助向量$\hat{H}_0$、$\hat{H}_1$，最终生成密文和共享密钥。保证了即使在量子计算环境下，攻击者也无法从密文中有效恢复共享密钥。
...
密文解封装算法（Algorithm \ref{alg:decap}）由接收方执行，利用私钥验证密文并恢复共享密钥。首先利用私钥中的标志位$b$确定要处理的密文部分，然后利用密钥矩阵$\mathbf{Z}_b$进行解码。关键步骤是恢复随机种子$r$并对密文进行验证，确保其有效性和完整性，不仅检验了密文的有效性，还隐式地实现了身份认证。
